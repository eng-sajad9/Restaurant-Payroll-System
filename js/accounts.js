...
function renderAccountsTable(list) {
    ...
    tbody.innerHTML = list.map(acc => {
        ...
        const createdAt = acc.created_at ? new Date(acc.created_at).toLocaleDateString('ar-SA', {
        ...
        return `<tr ${isSelf ? 'style="background:rgba(37,99,235,.04);"' : ''}>
          ...
          <td>
            <div class="tbl-actions">
              <button class="act-btn view" title="تعديل الدور" onclick="openAccModal('${acc.id}')">
                <svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                الدور
              </button>
              <button class="act-btn warn" title="تغيير كلمة المرور" onclick="openChangePwd('${acc.id}')">
                <svg viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                كلمة المرور
              </button>
              ${!isSelf ? `
              <button class="act-btn delete" title="حذف الحساب" onclick="deleteAccount('${acc.id}')">
...
function openAccModal(id = null) {
    ...
    if (id) {
        ...
        // Populate permissions for Manager
        if (acc.role === 'manager') {
            document.getElementById('perm-analytics').checked = acc.can_view_analytics !== false; // Default true
            document.getElementById('perm-delete').checked = !!acc.can_delete; // Default false
        }
    ...
    toggleAccPermissions(); // Ensure permissions group is correctly shown/hidden
}

/**
 * Show/Hide permissions group based on selected role
 */
function toggleAccPermissions() {
    const role = document.getElementById('acc-role').value;
    const permGroup = document.getElementById('acc-permissions-group');
    if (permGroup) {
        permGroup.style.display = (role === 'manager') ? 'block' : 'none';
    }
}
...
async function saveAccount() {
    ...
    // Collect permissions if manager
    const perms = {};
    if (role === 'manager') {
        perms.can_view_analytics = document.getElementById('perm-analytics').checked;
        perms.can_delete = document.getElementById('perm-delete').checked;
    }
...
async function saveChangePwd() {
    ...
    try {
        ...
        if (isSelf) {
            ...
            const { data: userDoc, error: userErr } = await supabase.from('users').select('password_hash').eq('id', current.id).single();
            if (userErr || !userDoc || userDoc.password_hash !== oldHash) {
