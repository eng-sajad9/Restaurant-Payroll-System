...
function renderDriverTable(drivers, records) {
    ...
    const rows = drivers.map((drv, idx) => {
        ...
        return `<tr data-drv-id="${drv.id}" data-drv-name="${escHtml(drv.name)}" class="${rec ? 'paid-row' : ''}">
          ...
          <td data-label="الوردية">
            ${rec?.shift_type
                ? `<span class="badge ${rec.shift_type === 'مسائي' ? 'badge-purple' : 'badge-blue'}" style="font-size:11px;">${rec.shift_type}</span>`
          ...
          <td>
            <div class="tbl-actions">
              ...
              ${rec ? `<button class="act-btn delete" title="حذف" onclick="deleteDrvRecord('${rec.id}')">
                <svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>
              </button>` : ''}
              <button class="act-btn delete" title="حذف السائق نهائياً" style="opacity:0.6;" onclick="deleteDriverProfile('${drv.id}', '${escHtml(drv.name)}')">
...
function loadDriverBaseSalary() {
    ...
    if (emp && document.getElementById('drv-base-salary')) {
        document.getElementById('drv-base-salary').value = Number(emp.base_salary || 0).toLocaleString('en-US');
    } else if (document.getElementById('drv-base-salary')) {
        document.getElementById('drv-base-salary').value = '0';
    }
}
...
async function saveNewDriver() {
    ...
    try {
        ...
        showToast('فشل إضافة السائق. تحقق من الاتصال بالإنترنت.', 'error');
    } finally {
        saveBtn.disabled = false;
    }
}
...
async function updateDriverProfile(driverId) {
    ...
    try {
        await db.collection('drivers').doc(driverId).update({ name, base_salary: salary, default_shift: defaultShift });
        ...
        showToast('فشل تحديث بيانات السائق.', 'error');
    } finally {
        saveBtn.disabled = false;
    }
}
